...
@tool(
    ...
    properties  = {"location": prop("string", "City or location e.g. Karachi, London")},  # ✅ use prop()
    required    = ["location"]
)
...
"I need a prompt for an AI agent to create a similar website builder with the above recommendations like lovabe.dev. The prompt should be like a director who is shooting a movie. make the prompt AI Agent understands better not miss any thing all everything making it much more powerful than lovable. also make the AI Agent to craete full stack application from a single prompt like create netflix clone etc . Also handle context window for AI RAG etc. Every thing you know just go."